...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.babelswap.com - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.babelswap.com)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://app.babelswap.com'}
          color={text === 'https://app.babelswap.com' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
